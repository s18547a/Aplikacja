...
exports.updateSurgeryReport=async(surgeryReport)=>{
  ...
  try {
    ...
    let pool =await sql.connect(config);
    let results = await pool.request().input("SurgeryId",sql.VarChar,surgeryId).input("Report",sql.VarChar,Report).query("Update Surgery Set Report=@Report Where SurgeryId=@SurgeryId")
