...
export const getVet= async (req, res) => {
  ...
  let results = await VetRepository.getVet(VetId);
...
export const getVets= async (req, res) => {
  ...
  let results = await VetRepository.getVets(parameters);
...
export const getVetTypes=async (req, res) => {
  ...
  let results = await VetTypeRepository.getVetTypes(parameters);
...
export const registerVet=  async (req, res) => {
  ...
  let results = await VetRepository.registerVet(newVet);
  if (results instanceof Error) {
    return res.status(500).json({});
  } else if (results == null) {
    return res.status(404).json({});
...
export const getAvailableHours=async (req, res) => {
  ...
  let results = await VetScheduldeRepository.getAvailableHours(parameters);
